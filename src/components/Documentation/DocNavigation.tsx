        ...
        <div className="flex flex-col gap-0.5 mt-4 items-start fixed pe-4" >
            {
                DocsNav.map((item) => {
                    return (
                        <Link key={item.id} href={`#${item.hash}`} onClick={() => getNavItem(item.hash)} className={`py-2.5 hover:bg-[#EFBF04]/20 hover:text-[#EFBF04] dark:hover:text-[#EFBF04] xl:min-w-60 lg:min-w-52 min-w-full px-4 rounded-md text-midnight_text text-base font-medium  ${item.hash === navItem ? "bg-[#EFBF04] text-white hover:!bg-[#EFBF04] hover:!text-white dark:!text-opacity-100 dark:hover:text-white" : "dark:text-white dark:text-opacity-60"}`}>{item.navItem}</Link>
                    )
                })
            }
        </div>
